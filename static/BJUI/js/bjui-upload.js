...
+function ($) {
    ...
    Upload.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            renderQueueItem: function(file) {
                ...
                $temp.on('upload.confirm', '.up_confirm', function(e) {
                    var $this = $(this), $queue = $this.closest('.item')
                    
                    $this.hide().next().show()
                    that.fileUpload($queue.data('upfile'), uploadedSize)
                })
                $temp.on('upload.cancel', '.up_cancel', this.removeQueueItem)
            },
    ...
    Upload.prototype.init = function() {
        ...
        //do select files
        $element
            .on('upload', '.bjui-upload-select-file', function(e) {
                that.fileSelect(e)
            })
            .on('upload', '.bjui-upload-select', function(e) {
    ...
    Upload.prototype.fileUpload = function(file, uploadedSize) {
        ...
        $element
            ...
            .off('upload.cancel')
            .on('upload.cancel', $.proxy(function(e) {
