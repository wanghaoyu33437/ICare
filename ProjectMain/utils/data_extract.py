    ...
    for pcap in PCAPS:
        if pcap.haslayer(TCP) and pcap.haslayer(Raw):
            ...
            if sport == 80 or sport == 8080:
                ...
                pass
        id += 1
    ip_port_ids_dict = dict()    #{'192.134.13.234:232':[2,3,4,5],'192.134.13.234:236':[4,3,2,4,3]}
    ...
    for pcap in PCAPS:
        if pcap.haslayer(TCP) and pcap.haslayer(Raw):
            ...
            if sport == tfport:
                ...
                pass
        id += 1
    ip_port_ids_dict = dict()    #{'192.134.13.234:232':[2,3,4,5],'192.134.13.234:232':[4,3,2,4,3]}
    for ip_port_id in ip_port_id_list:
        if ip_port_id['ip_port'] in ip_port_ids_dict:
            ip_port_ids_dict[ip_port_id['ip_port']].append(ip_port_id['id'])#PCAPS[ip_port_id['id']].load)
        else:
...
def client_info(PCAPS):
    with open('ProjectMain/utils/warning/CLIENT_INFO', 'r', encoding='UTF-8') as f:
        lines = f.readlines()
...
def sen_data(PCAPS, host_ip):
    ...
    for telnet in telnetdata:
        ...
        if restp.strip():
            data_id += 1
            sendata_list.append({'data_id':data_id,'ip_port': telnet['ip_port'], 'result': result, 'data':telnet['data']})
    #FTP协议帐号密码
    ...
    for ftp in ftpdata:
        ...
        if restp.strip():
            data_id += 1
            sendata_list.append({'data_id':data_id,'ip_port': ftp['ip_port'], 'result': result, 'data':data})
    ...
    for mail in maildata:
        ...
        if result.strip():
            data_id += 1
            sendata_list.append({'data_id':data_id,'ip_port': mail['ip_port'], 'result': result, 'data':data})

    #HTTP协议帐号密码
    with open('ProjectMain/utils/warning/HTTP_DATA', 'r', encoding='UTF-8') as f:
        lines = f.readlines()
    user = lines[0].strip()
    ...
    for web in webdata:
        ...
        if restp.strip():
            data_id += 1
            sendata_list.append({'data_id':data_id,'ip_port': web['ip_port'], 'result': result, 'data':data})
    return sendata_list