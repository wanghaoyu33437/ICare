...
import collections
import binascii
from ProjectMain.utils.data_extract import web_data, telnet_ftp_data

#根据可疑端口判断是否有木马病毒
def port_warning(PCAPS, host_ip):
    with open('ProjectMain/utils/protocol/WARN', 'r', encoding='UTF-8') as f:
        warns = f.readlines()
    WARN_DICT = dict()
    for warn in warns:
        warn = warn.strip()
...
def web_warning(PCAPS, host_ip):
    with open('ProjectMain/utils/warning/HTTP_ATTACK', 'r', encoding='UTF-8') as f:
        attacks = f.readlines()
    ATTACK_DICT = dict()
...
def ftp_warning(PCAPS, host_ip):
    ...
    ip_count = collections.Counter(ftp503_list)  # 将统一对象出现的次数以字典形式保存{object:？出现次数}
    warn_ip = {k:y for k,y in ip_count.items() if y>10}
    for ip, count in warn_ip.items():
        ftpwarn_list.append({'ip_port': ip, 'warn':u'FTP暴力破解', 'time': str(count), 'data':None}) # time表示次数
    return ftpwarn_list
...
def arp_warning(PCAPS):
    ...
    arp_list = list()
    for pcap in PCAPS:
        if pcap.haslayer(ARP) and pcap.getlayer(ARP).op == 2: # op=1请求 op=2应答
...
def exception_warning(PCAPS, host_ip):
    ...
    if port_list:
        warn_list.extend(port_list)
    if arp_list:
        warn_list.extend(arp_list)
    return warn_list